...
namespace SX.Math.Lexems.Values
{
    public class LexemValueStruct : LexemValue
    {
        ...
        public IList<LexemVariable> Members { get => _members.AsReadOnly(); }
        ...
        public override LexemVariable Execute(Lexem lexem, ILexemEnvironment? environment = null)
        {
            ...
            if (lexem is LexemVariable lexemVariable)
                return this[lexemVariable.Name];

            return base.Execute(lexem, environment);
        }

        public static LexemValueStruct? ParseValueStruct(ref string? text)
        {
