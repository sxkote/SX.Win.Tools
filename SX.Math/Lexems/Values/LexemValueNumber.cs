...
namespace SX.Math.Lexems.Values
{
    ...
    {
        public const string PATTERN = @"(?<number>(\d+[\.\,]?\d*|[\.\,]\d+))(?<exp>[eE](?<expsign>[\-\+])?(?<expnum>\d+)?)?";

        static public LexemValueNumber ZERO { get => new LexemValueNumber(0); }
        ...
        static private double CreateNumberFromRegexMatch(Match match)
        {
            ...
            try
            {
                ...
                if (match.Groups["exp"].Success)
                {
                    ...
                    result *= System.Math.Abs(System.Math.Pow(10, expnum));
                }
        ...
        static public double ParseNumber(string text, bool signed = false)
        {
            ...
            var pattern = String.Format("{0}{1}", signed ? @"(?<sign>[\-\+])?" : "", PATTERN);
        ...
        static public LexemValueNumber? ParseValueNumber(ref string? text)
        {
        ...
        #region Operators
        public static implicit operator LexemValueNumber(decimal number) => new LexemValueNumber((double)number);

        public static implicit operator LexemValueNumber(double number) => new LexemValueNumber(number);

        public static implicit operator LexemValueNumber(int number) => new LexemValueNumber((double)number);

        public static LexemValueNumber operator +(LexemValueNumber argument, LexemValueNumber number) => argument.Value + number.Value;
        public static LexemValueNumber operator -(LexemValueNumber argument, LexemValueNumber number) => argument.Value - number.Value;
        public static LexemValueNumber operator *(LexemValueNumber argument, LexemValueNumber number) => argument.Value * number.Value;
        public static LexemValueNumber operator /(LexemValueNumber argument, LexemValueNumber number) => argument.Value / number.Value;
        ...
        public static bool operator !=(LexemValueNumber argument, LexemValueNumber number) => !(argument == number);
        public static bool operator >(LexemValueNumber argument, LexemValueNumber number)
        {
        ...
        public static bool operator <(LexemValueNumber argument, LexemValueNumber number) => number > argument;
        public static bool operator >=(LexemValueNumber argument, LexemValueNumber number)
        {
            if (argument == null || number == null)
                return false;
